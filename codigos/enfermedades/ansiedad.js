...
function inicializarAudioAnsiedad() {
  ...
  try {
    ...
    beatEl.preload = 'auto';
    beatEl.loop = true;
    beatEl.addEventListener('canplaythrough', () => console.log("[Ansiedad] latidos listo"));
    beatEl.addEventListener('play',  () => console.log("[Ansiedad] latidos PLAY"));
    beatEl.addEventListener('pause', () => console.log("[Ansiedad] latidos PAUSE"));
    beatEl.addEventListener('error', (e) => console.warn("[Ansiedad] latidos error", e));
    ...
    breathEl.loop = true;
    breathEl.addEventListener('canplaythrough', () => console.log("[Ansiedad] respiración lista"));
    breathEl.addEventListener('play',  () => console.log("[Ansiedad] respiración PLAY"));
    breathEl.addEventListener('pause', () => console.log("[Ansiedad] respiración PAUSE"));
    breathEl.addEventListener('error', (e) => console.warn("[Ansiedad] respiración error", e));

    // Gains por pista (balance fino)
    const beatGain = audioCtx.createGain();   beatGain.gain.value = 0.95;
    const breathGain = audioCtx.createGain(); breathGain.gain.value = 0.75;
  ...
  const unlock = () => {
    ...
    if (audioCtx && audioCtx.state === 'suspended') {
      audioCtx.resume()
        .then(() => {
          audioUnlocked = true;
          console.log("[Ansiedad] AudioContext desbloqueado tras gesto del usuario.");
        })
...
function stopAnsiedad(camara) {
  ...
  try { if (beatEl) { beatEl.pause(); beatEl.currentTime = 0; } } catch {}
  try { if (breathEl) { breathEl.pause(); breathEl.currentTime = 0; } } catch {}
}
...
function activarSistemaAnsiedad() {
  ...
  if (visible) {
    console.log("[Ansiedad] esperando a que se oculte pantalla de carga para armar gate…");
    const onHide = () => {
